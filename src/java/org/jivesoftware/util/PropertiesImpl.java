...
/**
 * PropertiesImpl 针对Properties进行扩展的工具类
 * 
 * 扩展的两个主要功能: 1.对Properties文件中注释的保存。 PropertiesFullUtil在读取和保存Properties文件时,会将其注释
 * 一起读取保存。PropertiesImpl中会提供方法来根据key获取
 * 相应注释。在PropertiesImpl中添加一个K-V对时，也会提供 添加相应注释的方法。
 * 
 * 2.对Properties文件中Key值顺序的保证。 PropertiesFullUtil会保证Key的顺序。从一个Properties文件中
 * 读取所有K-V对，保存到另一个Properties文件时，Key的顺序不会 改变。
 *
 */
